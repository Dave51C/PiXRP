...
from PiggyVisionXRP import TAG_SPECS, rotate, Reef, ROBOT_OFFSET, Crosshairs
...
'''
if Logging == 'True':
    from Logging import open_logfile, write_logfile
    try:
        LogName = LogDir+time.strftime('%Y%m%d_%H%M%S')+'.log'
        logfile = open_logfile(LogName)
        print('Log file opened.')
        write_logfile(logfile,'Starting\n')
    except:
        Logging = 'False'
...
if __name__ == "__main__":
    ...
    if server:
        ...
        #pubFrameNbr  = CamPos_tbl.getDoubleTopic("FrameNbr").publish()
    ...
    imgBuf = np.zeros(shape=(CamMtx[0]['height'], CamMtx[0]['width'], 3), dtype=np.uint8)
    ...
    input_stream  = CameraServer.getVideo(CamMtx[0]['name'])
    output_stream = CameraServer.putVideo(CameraLabel, CamMtx[0]['width'], CamMtx[0]['height'])

    options  = apriltag.DetectorOptions(
       families="tag36h11"
       ,nthreads      = 4
       ,quad_decimate = CamDetOpts[0]["quad_decimate"]
       ,quad_blur     = CamDetOpts[0]["quad_blur"]
       ,refine_edges  = CamDetOpts[0]["refine_edges"]
       ,refine_decode = CamDetOpts[0]["refine_decode"]
       ,refine_pose   = CamDetOpts[0]["refine_pose"]
       ,quad_contours = CamDetOpts[0]["quad_contours"]
       )
    detectors = apriltag.Detector(options)
...
while True:
    #if Logging == 'True':
        #write_logfile(logfile,f"main loop,{time.time():.4f}\n")
    All = {}
    TagsFound = []
    DSview = subDS.get('0')
    if DSview in ('1','3'):
    ...
    N=0
    #if Logging == 'True':
        #write_logfile(logfile,f"{N:d},{time.time():.4f}\n")
    frame_time, input_img = input_stream.grabFrame(imgBuf)
    output_img= np.copy(input_img)
    gray = cv2.cvtColor (input_img, cv2.COLOR_BGR2GRAY)
    results = detectors.detect(gray)
    InFrame = Find_Apriltags (GameTags, output_img, results, CamMtx[0]['mtx'],CamMtx[0]['dist'],CamMtx[0]['FudgeOffset'],CamMtx[0]['FudgeFactor'],N)
    ...
    if DSview == '0':
        ...
        Crosshairs(output_img,CamMtx[0],
        CamMtx[0]["width"], CamMtx[0]["height"])
    elif DSview == '8':
        Smiley(output_img,CamMtx[0])
    else:
        pass

    output_img = cv2.flip(output_img,-1)
    output_stream.putFrame(output_img)
    #pubFrameNbr.set(FrameNbr)
    FrameNbr += 1
    #output_stream.putFrame(output_img)
    #if Logging == 'True':
        #write_logfile(logfile,f"S,{time.time():.4f}\n")
